...
\item We present experimental data 
gathered in the process of writing the linking spec, 
illustrating that FIXME (hope for significant discrepancy).
TWO experiments: symbol resolution ``defined where, referenced where'' 
(multiple definitions make life interesting; more for dynamic linking than static, so 
may or may not be a goer for this paper); ``defined how, referenced how'', i.e.\ symbol flavour
interactions (should at least turn up the ``undefined weak thread-local'' problems in amd64, 
hopefully others).
