...
explicitly allow substitution: if the user supplies their own \textsf{malloc.o} and also links in \textsf{libc.a}, the former's definitions taking precedence over any those in the archive.
...
This environment variable can supply a named library, such that \emph{all} its definitions take precedence over those in all other libraries (but not over the executable!). 
...
%it has a clear meaning at the linker level: for any given definition,
...
% Linker-speak has various other roles.
% Linker scripts can specify header content in the output file, allowing
% communication between linker and loader.
% This is used on GNU/Linux to memory-protect additional regions of the image. 
% Hooks in dynamically linked programs allow the debugger to be notified of 
% Dynamic linkers communicate  
...
\sk{Seems missing an explanation of why we care. Mainly we don't, but
worth mentioning these because if we don't, people will ask why not.}
