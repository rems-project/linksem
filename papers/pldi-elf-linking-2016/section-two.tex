...
\item[Command-line arguments] ---supplied when invoking the linker.

\item[Linker scripts] ---written in the linker's script language. 
...
\item[Object file features] ---largely meaning symbols, sections, relocations and their
attributes affecting link semantics. Many of these attributes 
have corresponding forms in assemblers (``directives'' or pseudo-ops) 
and compilers (attributes) allowing the programmer
to control the object file that is output.

\item[Object file usage conventions] As a rule, 
section names are formally meaningless to the linker.
However, ABIs often attach meaning to specific section names,
require the presence of certain named sections, and so on.
(Additionally, some linker features flout this rule by attaching meaning to section names.)
...
allow similar name-visibility control but at the coarser granularity of 
...
since calls apparently targeting a dynamic object 
(such as \textsf{libc.so}) might actually have been bound statically
in some libraries or versions thereof
...
In these cases, dynamic interposition will fail.
...
provides this feature when generating System~V shared libraries.
...
The latter is often used to dynamically 
...
However, finer-grain interposition can also be used to achieve substitution.
...
However, it si completely valid ELF to instead encode it as a pair of like-named symbols, 
