...
imports
  ...
  (* X86 model imports *)
  "x86_64_model/L3_Theory"
  "x86_64_model/X64"
  (* ELF model imports *)
  "auto_generated/isabelle-2016/Import_everything"
  "Termination"
  "auto_generated/isabelle-2016/Test_image"
  "$ISABELLE_HOME/src/HOL/Eisbach/Eisbach"
  "$ISABELLE_HOME/src/HOL/Word/WordBitwise"
begin
...
lemma valid_address_signed_arith:
  ...
  shows "valid_address (4194316::nat) (8::nat) addr \<Longrightarrow>
    ((18446744071562067968::64 word) <=s ((of_nat addr)::64 word) \<and> ((of_nat addr)::64 word) <=s (2147483647::64 word))"
  apply auto
...
lemma nth_the_technical:
  assumes "(4194316::nat) \<le> x" and "x < 4194324"
...
lemma img1_technical:
  ...
  shows "img1 offset [72, 199, 4, 37, 0, 0, 0, 0, 5, 0, 0, 0] =
          \<lparr>elements = [''.data'' \<mapsto> \<lparr>startpos = Some 4194316, length1 = Some 8, contents = [Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0]\<rparr>,
...
lemma set_comprehension_collapse_miss_lower:
  ...
  shows "{s. \<exists>k\<in>{''.data'', ''.text''}.
                      Some s = [''.data'' \<mapsto> \<lparr>startpos = Some 4194316, length1 = Some 8, contents = [Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0]\<rparr>, ''.text'' \<mapsto>
...
lemma set_comprehension_elim_text_technical:
  assumes "4194304 \<le> x" and "x < 4194316"
  shows "{s. \<exists>k\<in>{''.data'', ''.text''}.
          Some s = [''.data'' \<mapsto> \<lparr>startpos = Some 4194316, length1 = Some 8, contents = [Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0]\<rparr>,
                    ''.text'' \<mapsto> \<lparr>startpos = Some 4194304, length1 = Some 12,
                                  contents = (([Some 72, Some 199, Some 4, Some 37] @ foldr (\<lambda>b. op # (Some b)) (natural_to_le_byte_list (4194316 + (addr - 4194316))) []) @
                                              replicate (4 - length (natural_to_le_byte_list (4194316 + (addr - 4194316)))) (Some (of_nat 0))) @ [Some 5, Some 0, Some 0, Some 0]\<rparr>]
                      k \<and> the (startpos s) \<le> x \<and> x < the (startpos s) + the (length1 s)} = {\<lparr>startpos = Some 4194304, length1 = Some 12,
                                  contents = (([Some 72, Some 199, Some 4, Some 37] @ foldr (\<lambda>b. op # (Some b)) (natural_to_le_byte_list (4194316 + (addr - 4194316))) []) @
                                              replicate (4 - length (natural_to_le_byte_list (4194316 + (addr - 4194316)))) (Some (of_nat 0))) @ [Some 5, Some 0, Some 0, Some 0]\<rparr>}"
using assms
  apply(auto simp del: foldr.simps natural_to_le_byte_list.simps)
...
lemma set_comprehension_elim_data_technical:
  assumes "4194316 \<le> x" and "x < 4194324"
  shows "{s. \<exists>k\<in>{''.data'', ''.text''}.
                      Some s = [''.data'' \<mapsto> \<lparr>startpos = Some 4194316, length1 = Some 8, contents = [Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0]\<rparr>, ''.text'' \<mapsto>
                                \<lparr>startpos = Some 4194304, length1 = Some 12,
                                   contents = (([Some 72, Some 199, Some 4, Some 37] @ foldr (\<lambda>b. op # (Some b)) (natural_to_le_byte_list (4194316 + (addr - 4194316))) []) @
                                              ...
                                              [Some 5, Some 0, Some 0, Some 0]\<rparr>]
                                k \<and>
                      the (startpos s) \<le> unat ((of_nat x)::64 word) \<and> unat ((of_nat x)::64 word) < the (startpos s) + the (length1 s)} = {\<lparr>startpos = Some 4194316, length1 = Some 8, contents = [Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0]\<rparr>}"
using assms
  apply(auto simp del: foldr.simps natural_to_le_byte_list.simps)
...
lemma set_comprehension_elim_neither_technical:
  shows "4194324 \<le> (x::nat) \<Longrightarrow> (x::nat) < 18446744073709551616 \<Longrightarrow> {s. \<exists>k\<in>{''.data'', ''.text''}.
                      Some s = [''.data'' \<mapsto> \<lparr>startpos = Some 4194316, length1 = Some 8, contents = [Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0, Some 0]\<rparr>, ''.text'' \<mapsto>
                                \<lparr>startpos = Some 4194304, length1 = Some 12,
                                   contents = (([Some 72, Some 199, Some 4, Some 37] @ foldr (\<lambda>b. op # (Some b)) (natural_to_le_byte_list (4194316 + (addr - 4194316))) []) @
                                               replicate (4 - length (natural_to_le_byte_list (4194316 + (addr - 4194316)))) (Some (of_nat 0))) @
...
lemma list_eq_discharge_technical:
  assumes "valid_address 4194316 8 addr" and "4194304 \<le> x \<and> x < 4194316"
  shows "[72, 199, 4, 37, word_extract 7 0 ((of_nat addr)::64 word), word_extract 15 8 ((of_nat addr)::64 word), word_extract 23 16 ((of_nat addr)::64 word), word_extract 31 24 ((of_nat addr)::64 word), 5, 0, 0, 0] ! (x - 4194304) =
       the (((([Some 72, Some 199, Some 4, Some 37] @ foldr (\<lambda>b. op # (Some b)) (natural_to_le_byte_list (4194316 + (addr - 4194316))) []) @
              replicate (4 - length (natural_to_le_byte_list (4194316 + (addr - 4194316)))) (Some (of_nat 0))) @
