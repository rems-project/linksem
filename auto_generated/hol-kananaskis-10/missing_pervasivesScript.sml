...
val _ = Define `
 (hex_string_of_byte b=    
  (IMPLODE [ hex_char_of_nibble ((w2n b) DIV I 16)
             ; hex_char_of_nibble ((w2n b) MOD I 16)]))`;
...
(*val natural_of_decimal_string_helper : natural -> list char -> natural*)
 ...
 ((case chars of 
        ...
        | c :: cs => (case natural_of_decimal_digit c of
            SOME dig => natural_of_decimal_string_helper ((I 10 * acc) + dig) cs
            | NONE => acc
        )
    )))`;
...
val _ = Define `
 (natural_of_decimal_string s=    
  (natural_of_decimal_string_helper(I 0) (EXPLODE s)))`;
...
(*val hex_string_of_natural : natural -> string*)
 ...
 (hex_string_of_natural n=    
  (if n <I 16 then IMPLODE [hex_char_of_nibble n]
    else  STRCAT(hex_string_of_natural (n DIV I 16)) (IMPLODE [hex_char_of_nibble (n MOD I 16)])))`;
...
(*val sort_by : forall 'a. ('a -> 'a -> ordering) -> list 'a -> list 'a*)
 ...
 ((case xs of
    ...
    | [x] => [x]
    | xs =>
      let ls = (TAKE (LENGTH xs DIV I 2) xs) in
      let rs = (DROP (LENGTH xs DIV I 2) xs) in
...
(*val mapMaybei' : forall 'a 'b. (natural -> 'a -> maybe 'b) -> natural -> list 'a -> list 'b*)
 ...
 ((case xs of
  ...
  | x::xs =>
      (case f idx x of
        NONE => mapMaybei' f (I 1 + idx) xs
      | SOME e  => e :: mapMaybei' f (I 1 + idx) xs
      )
  )))`;
        ...
        | i :: more_is => 
            ...
            (case right of 
                ...
                | x :: more_xs => 
                    ...
                    partitionii' (i+I 1) more_is more_xs ((i, x) :: reverse_accum) 
...
val _ = Define `
 (filteri is xs=    
  (let sorted_is = (QSORT (<=) is) in
    let (accum, accum_compl) = (partitionii'(I 0) sorted_is xs [] [])
    in 
...
val _ = Define `
 (string_index_of e s=  (string_index_of' e (EXPLODE s)(I 0)))`;
 ...
 ((case xs of
		...
		| y::ys =>
			if p y then
				...
				find_index_helper (count1 +I 1) p ys
	)))`;
...
val _ = Define `
 (find_index0 p xs=  (find_index_helper(I 0) p xs))`;
...
(*val replicate_revacc : forall 'a. list 'a -> natural -> 'a -> list 'a*)
 ...
 ((case len of
		  0 => REVERSE revacc
		| m => replicate_revacc (e :: revacc) (m - I 1) e
	)))`;
  ...
  (*  let _ = Missing_pervasives.errs ("Taking a byte; have accumulated " ^ (show (List.length acc) ^ " so far\n"))
   in *)(case n of
      ...
      | _ => (case l of
            ...
            | x :: xs => list_take_with_accum (n - I 1) (x :: reverse_acc) xs
        )
    )))`;
...
val _ = Define `
 (padding_and_maybe_newline c width str=    
  (let padlen = (width - (I (STRLEN str))) in
    
     STRCAT(if padlen <=I 1 then "\n" else "") (IMPLODE (replicate (if padlen <=I 1 then width else padlen) c))))`;
...
val _ = Define `
 (padding_to c width str=    
  (let padlen = (width - (I (STRLEN str))) in
    if padlen <=I 0 then "" else (IMPLODE (replicate padlen c))))`;
